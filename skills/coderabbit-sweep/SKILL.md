...
and `pulls/<n>/comments` stays empty. On the SHA test alone such a PR reads as *never reviewed*
forever, so the sweep re-fires it every cooldown and burns a slot re-reviewing work already done.
Observed on `jfreal/auxf#182`, 2026-08-23: *"Full review finished."* at 18:14:44Z, walkthrough
posted, zero reviews. Before calling a PR never-reviewed, check the issue comments for either
signal below; **complete** if either names the head SHA or follows the newest push:

```
gh api "repos/<slug>/issues/<n>/comments" --jq '.[] | select(.user.login|test("coderabbit";"i")) | select(.body|test("Full review finished|Review completed")) | .updated_at'
gh api "repos/<slug>/issues/<n>/comments" --jq '.[] | select(.user.login|test("coderabbit";"i")) | select(.body|test("walkthrough_start")) | .body' | grep -o "and [0-9a-f]\{40\}"
```

The walkthrough's `Reviewing files that changed from the base of the PR and between <base> and
<head>.` line carries the reviewed head SHA, which is the sturdier of the two — the reply comment
is worded by the vendor and has changed before.
...
- **`reviewed`** — a new `Actionable comments posted` review at head SHA, **or** the zero-findings
  ...
  replaced by a walkthrough naming the head SHA. Clear `throttledUntil`.
